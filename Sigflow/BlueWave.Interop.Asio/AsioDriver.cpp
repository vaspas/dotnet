	...
	namespace Interop
	{
		namespace Asio
		{
			...
			void AsioDriver::CreateBuffers(int bufferSize)
			{
				...
				_bufferSizeInput=bufferSize;
				_bufferSizeOutput=bufferSize;
			...
			void AsioDriver::CreateInputBuffers(int bufferSize)
			{
				// create our input and output channel arrays
				_inputChannels = gcnew array<Channel^>(_nInputs);
				...
				// each channel needs a buffer info
				ASIOBufferInfo* pBufferInfos = new ASIOBufferInfo[_nInputs];
				...
				// get the driver to create its buffers
				_pDriver->createBuffers(pBufferInfos, _nInputs, bufferSize, _pCallbacks);
			...
			void AsioDriver::CreateOutputBuffers(int bufferSize)
			{
				// create our input and output channel arrays
				_outputChannels = gcnew array<Channel^>(_nOutputs);
				...
				// each channel needs a buffer info
				ASIOBufferInfo* pBufferInfos = new ASIOBufferInfo[_nOutputs];
				
				// and do the same for output channels
				for (int index = 0; index < _nOutputs; index++)
				{
					pBufferInfos[index].isInput = 0;
					pBufferInfos[index].channelNum = index;
					pBufferInfos[index].buffers[0] = 0;
					pBufferInfos[index].buffers[1] = 0;
				}
				...
				// get the driver to create its buffers
				_pDriver->createBuffers(pBufferInfos, _nOutputs, bufferSize, _pCallbacks);
			...
			int AsioDriver::BufferSizeInput::get()
			{
				return _bufferSizeInput;
			}

			int AsioDriver::BufferSizeOutput::get()
			{
				return _bufferSizeOutput;
			}
